...
def img_transform(img_in, transform):
    ...
    img = transform(img)
    img = img.unsqueeze(0)    # 增加维度(0表示,在第一个位置增加维度) C*H*W --> B*C*H*W
    return img
...
if __name__ == '__main__':
    ...
    path_img = os.path.join(BASE_DIR, "grad_cam_data", "cam_img", "test_img_7.png")
    ...
    # 生成cam
    grads_val = grad_block[0].cpu().data.numpy().squeeze() #这个方法可以去除数组中长度为1的维度
