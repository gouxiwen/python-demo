...
def main(args):
    ...
    # @note
    # 确定性算法，如果使用了，则禁用cudnn的benchmark模式
    # 随着数据越来越大，Deterministic algorithms规则已经越来变得越来越不适用
    ...
    # @note: 自动混合精度训练，混合精度训练是一种使用两种数据类型（通常是 float16 和 float32）来加速训练过程并减少内存使用量的技术
    ...
    # @note: 预热学习率，它的主要目的是在训练初期将学习率从较低值逐渐提高到设定的初始学习率，以避免模型在初始训练阶段由于学习率过高而出现的不稳定情况。
    ...
    # @note: 如果使用分布式训练，则将模型包装为分布式数据并行模型
    # if args.distributed:
    #     model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu])
    #     model_without_ddp = model.module

    model_ema = None
    #@note: 如果使用模型指数移动平均，则创建一个指数移动平均模型,可以帮助模型更稳定地收敛，并提高泛化能力
    # if args.model_ema:
